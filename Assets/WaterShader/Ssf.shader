Shader "SampleSsf/Hidden/Ssf"
{
    ...
    SubShader
    {
        ...
        half3 SampleBox(float2 uv, float delta)
        {
            ...
            half3 s = Sample(uv + offset.xy) + Sample(uv + offset.zy) + Sample(uv + offset.xw) + Sample(uv + offset.zw);
            return s * 0.25f;
        }
        ...
        Pass
        {
            ...
            float4 DepthNormalPassFragment(Varyings input) : SV_Target
            {
                ...
                // スクリーン全体のuvと深度値を基にカメラ空間の座標を計算する
                float3 pos = ReconstructPosition(input.uv, depth);
                
                // 隣接ピクセルの座標を比較し、差分から法線を計算。
        ...
        Pass
        {
            ...
            half EdgeDetection(float2 uv[5], float3 viewDirWS)
            {
                ...
                float edgeNormal = sqrt(
                    dot(normalDifference1, normalDifference1) + dot(normalDifference2, normalDifference2));
